...
const titles = {
  dashboard:"Buenos días, Cristian", inventory:"Inventario", sales:"Registrar venta",
...
function inventoryView() {
  ...
  return `
    <div class="section-heading"><div><h2>Inventario</h2><p>${state.products.length} productos registrados · ${state.products.reduce((a,p)=>a+p.stock,0)} unidades disponibles</p></div><button class="primary-button" id="addProduct">＋ Agregar producto</button></div>
    ...
    <div class="toolbar"><input id="inventorySearch" value="${state.inventorySearch}" placeholder="Buscar por nombre, marca o código"><select id="categoryFilter">${categories.map(c=>`<option ${c===state.inventoryCategory?"selected":""}>${c}</option>`).join("")}</select></div>
    <div class="table-wrap"><table><thead><tr><th>Producto</th><th>Categoría</th><th>Costo</th><th>Precio</th><th>Margen</th><th>Stock</th><th></th></tr></thead><tbody>
      ${filtered.map(p=>`<tr><td><div class="product-cell"><div class="product-thumb ${colorClass(p.category)}">${initials(p.name)}</div><div><strong>${p.name} · ${p.ml} ml</strong><span>${p.id} · ${p.brand}</span></div></div></td><td><span class="category-pill">${p.category}</span></td><td>${money(p.cost)}</td><td><strong>${money(p.price)}</strong></td><td>${Math.round((p.price-p.cost)/p.price*100)}%</td><td><span class="stock-pill ${p.stock<=p.minStock?"low":"ok"}">${p.stock} un.</span></td><td><div class="row-actions"><button class="mini-button" data-edit-product="${p.id}">Editar</button></div></td></tr>`).join("") || `<tr><td colspan="7" class="empty-state">No se encontraron productos.</td></tr>`}
    </tbody></table></div>`;
}
  ...
  return `
    ...
    <div class="sales-layout">
      <section class="panel">
        ...
        <div class="product-picker">${products.map(p=>`
...
function ordersView() {
  return `
    ...
    ${state.orders.map(o=>`<article class="order-card"><div class="order-top"><div><span class="eyebrow">${o.id}</span><h3>${o.client}</h3></div>${statusPill(o.status)}</div><div class="order-meta"><span>Fecha: ${formatDate(o.date)}</span><span>Total: <strong>${money(o.total)}</strong></span><span>Pagado: ${money(o.paid)}</span><span>Saldo: ${money(o.total-o.paid)}</span></div><p class="order-products"><strong>${o.products}</strong><br><small>${o.address}</small></p><div class="row-actions"><button class="mini-button" data-order-next="${o.id}">Avanzar estado</button><button class="mini-button" data-order-payment="${o.id}">Registrar abono</button></div></article>`).join("")}`;
}
...
function suppliersView() {
  const suggested = state.products.filter(p=>p.stock<=p.minStock);
  return `
    ...
    <div class="content-grid">
      ...
      <aside class="panel"><div class="panel-header"><div><h3>Compra sugerida</h3><p class="panel-subtitle">Basada en stock mínimo</p></div></div><div class="alert-list">
        ${suggested.map(p=>`<div class="alert-item"><div class="product-thumb ${colorClass(p.category)}">${initials(p.name)}</div><div><strong>${p.name}</strong><span>${p.brand}</span></div><span class="stock-pill low">Pedir ${Math.max(6-p.stock,2)}</span></div>`).join("")}
      </div></aside>
    </div>`;
}
...
function render() {
  const views = {dashboard:dashboardView,inventory:inventoryView,sales:salesView,orders:ordersView,clients:clientsView,suppliers:suppliersView,finance:financeView,reports:reportsView};
...
function productForm(product={}) {
  return `<h2>${product.id?"Editar":"Agregar"} producto</h2><form id="productForm" class="form-grid">
    ...
    <div class="modal-actions full"><button type="button" class="secondary-button" data-close-modal>Cancelar</button><button class="primary-button">Guardar producto</button></div>
  </form>`;
}

function bindViewEvents() {
  document.querySelectorAll("[data-view-link]").forEach(el=>el.onclick=()=>setView(el.dataset.viewLink));
  document.querySelector("#addProduct")?.addEventListener("click",()=>openModal(productForm()));
  document.querySelectorAll("[data-edit-product]").forEach(el=>el.onclick=()=>openModal(productForm(state.products.find(p=>p.id===el.dataset.editProduct))));
  document.querySelector("#inventorySearch")?.addEventListener("input",e=>{state.inventorySearch=e.target.value; render(); document.querySelector("#inventorySearch")?.focus();});
...
function openOrderForm() {
  openModal(`<h2>Nuevo pedido</h2><form id="orderForm" class="form-grid"><div class="form-group full"><label>Cliente</label><input name="client" required></div><div class="form-group full"><label>Productos</label><textarea name="products" placeholder="Ej: 1 × Yara, 1 × Asad" required></textarea></div><div class="form-group"><label>Total</label><input name="total" type="number" step=".01" required></div><div class="form-group"><label>Abono recibido</label><input name="paid" type="number" step=".01" value="0"></div><div class="form-group full"><label>Dirección / entrega</label><input name="address" required></div><div class="modal-actions full"><button type="button" class="secondary-button" data-close-modal>Cancelar</button><button class="primary-button">Crear pedido</button></div></form>`);
}
    ...
    <form id="purchaseForm" class="form-grid">
      ...
      <div class="form-group full">
        ...
        <div class="purchase-lines" id="purchaseLines">
          ${(suggested.length?suggested.slice(0,5):state.products.slice(0,1)).map(p=>purchaseLineRow(p,Math.max(6-p.stock,2))).join("")}
        </div>
      </div>
...
function purchaseLineRow(product=state.products[0], qty=1) {
  return `<div class="purchase-line">
    ...
    <select name="ml" aria-label="Mililitros" required>${[30,50,60,100].map(ml=>`<option value="${ml}" ${Number(product.ml)===ml?"selected":""}>${ml} ml</option>`).join("")}</select>
...
function receivePurchase(id) {
  const order=state.purchaseOrders.find(o=>o.id===id); order.status="Recibido"; persist(); render(); showToast(`Orden ${id} recibida.`);
}
...
document.addEventListener("submit",e=>{
  ...
  if(e.target.id==="productForm"){
    if(data.id){Object.assign(state.products.find(p=>p.id===data.id),{...data,ml:+data.ml,cost:+data.cost,price:+data.price,stock:+data.stock,minStock:+data.minStock});}
    ...
    persist();closeModal();render();showToast("Producto guardado.");
  }
  ...
  if(e.target.id==="orderForm"){state.orders.unshift({id:`PED-${32+state.orders.length}`,...data,total:+data.total,paid:+data.paid,date:todayISO(),status:"Pendiente"});persist();closeModal();render();showToast("Pedido creado.");}
